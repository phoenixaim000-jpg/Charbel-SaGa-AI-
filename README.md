 [指定6つのGoogleアカウント / Gmail]  ────────┐
 [X (旧Twitter) AIM PHOENIX]          ├─► [Python統合AIエージェント]
 [CACAO屋RECORD & 各SNS/LINE]          │    ├─ AIシーナ（司令塔・進捗・厳しく管理）
 [猫チャメ (癒やし・会話・伴走)] ───────┘    ├─ チャメ（作業ペース計測・リラックス）
                                            ├─ ダブルブッキング検知＆統合判定エンジン
                                            └─ 作業スピード/余力予測AI (Predictor)
                                                      │
                                                      ▼
                                       【単一の主カレンダー (Primary Calendar)】
                                       ・メイン予定 / ミッション / クエスト
                                       ・競合/過去ログ/パラレル可能性のメタデータ保存
